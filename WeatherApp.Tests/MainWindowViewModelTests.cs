...
namespace WeatherApp.Tests
{
    public class MainWindowViewModelTests
    {
        ...
        {
            var weather = new Weather("TestCity", 10.0m, "clear", 50, 3.2m, "01d");
            ...
            string expectedWind = $"Wind: {3.2m} m/s";
            Assert.Equal(expectedTemp, vm.TemperatureDisplay);
            Assert.Equal("clear", vm.Description);
            Assert.Equal("Humidity: 50%", vm.HumidityDisplay);
            Assert.Equal(expectedWind, vm.WindSpeedDisplay);
        ...
        {
            var response = new GetWeatherResponse(true, new Weather("TestCity", 10.0m, "clear", 50, 3.2m, "01d"));
            var useCase = new FakeUseCase(response);
            var vm = new MainWindowViewModel(useCase);
            vm.City = "";
